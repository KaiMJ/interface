...
Control is a token with exactly one holder, and the *session outlives the transfer*: the human
gets the same display, the same Chromium process, the same cookies, the same half-filled form.

    AUTOMATION --(escalate)--> PENDING --(operator takes control)--> HUMAN
         ^                                                             |
         '------------------- (resume) <---- RELEASING <---------------'

Who holds it is explicit state, not the implicit consequence of "nobody is currently calling
page.click()" — without that, an operator clicking mid-run races the automation on one display.
"""
...
class InterventionRequest(BaseModel):
    """Raised when the system cannot safely proceed. Carries enough for an operator to act
    without reading logs: which capability and goal, which step, what it was trying to do,
    what it saw, and a live screenshot."""
...
class HumanAction(Frozen):
    ...
    Captured at the X layer, not by asking the operator what they did. Playwright cannot see a
    manual click — it did not issue it — so instrumenting the browser would leave a hole in the
    audit trail exactly where a human touched regulated data.
    """
...
class InterventionResolution(BaseModel):
    ...
    duration_ms: int = 0
    resolved_at: str = ""
    # On resume the runner re-observes rather than trusting a step counter: the human may have
    # advanced the app several screens.
